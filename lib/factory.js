...
function factory( options ) {
	...
	if ( arguments.length ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0oz47', options ) );
		}
		...
		if ( hasOwnProp( options, 'prng' ) ) {
			...
			if ( opts.prng === void 0 ) {
				throw new TypeError( format( '0oz7Y', 'prng', opts.prng ) );
			}
		} else if ( hasOwnProp( options, 'state' ) ) {
			opts.state = options.state;
			if ( opts.state === void 0 ) {
				throw new TypeError( format( '0oz7Y', 'state', opts.state ) );
			}
		} else if ( hasOwnProp( options, 'seed' ) ) {
			opts.seed = options.seed;
			if ( opts.seed === void 0 ) {
				throw new TypeError( format( '0oz7Y', 'seed', opts.seed ) );
			}
		}
		if ( hasOwnProp( options, 'copy' ) ) {
			opts.copy = options.copy;
			if ( !isBoolean( opts.copy ) ) {
				throw new TypeError( format( '0oz2o', 'copy', opts.copy ) );
			}
		}
	}
	prng = PRNGS[ opts.name ];
	if ( prng === void 0 ) {
		throw new Error( format( '0oz7Z', opts.name ) );
	}
	if ( opts.prng === void 0 ) {
		if ( opts.state === void 0 ) {
			if ( opts.seed === void 0 ) {
